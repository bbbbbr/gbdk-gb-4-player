...
//
uint8_t   _4p_rx_fifo[RX_FIFO_SZ];         // RX Buffer is double buffered to allow access of one while other is filling
uint8_t * _4p_rx_fifo_WRITE_ptr;           // Write pointer to SIO RX buf for incoming data
uint8_t * _4p_rx_fifo_READ_ptr;            // Read pointer to SIO RX buf for reading out the received data
uint8_t   _4p_rx_fifo_count;               // Number of bytes currently in SIO RX buf
#define                                    RX_FIFO_PTR_END_WRAP_ADDR (_4p_rx_fifo + RX_FIFO_SZ)  // Used by ISR for pointer wraparound
const uint8_t * _4p_rx_fifo_end_wrap_addr = RX_FIFO_PTR_END_WRAP_ADDR;                          // Used by Main for pointer wraparound
...
uint8_t four_player_rx_fifo_get_num_packets_ready(void) {
    ...
    // only use data if it's >= RX_FIFO_PACKET_SZ.
    //
    ...
    return _4p_rx_fifo_count / RX_FIFO_PACKET_SZ;
}
...
void four_player_rx_fifo_remove_n_packets(uint8_t packet_count_to_remove) {
    ...
    uint8_t byte_count_to_remove = packet_count_to_remove * RX_FIFO_PACKET_SZ;
    CRITICAL {
        if (byte_count_to_remove <= _4p_rx_fifo_count)
            _4p_rx_fifo_count -= byte_count_to_remove;
    }
}
...
static void sio_handle_mode_xfer(uint8_t sio_byte) {
    ...
    if (_4p_rx_fifo_count != RX_FIFO_SZ) {
        ...
        if (_4p_rx_packets_to_discard == NO_PACKETS_TO_DISARD) {
            ...
            *_4p_rx_fifo_WRITE_ptr++ = sio_byte;
            #ifdef DEBUG_SHOW_CHECKSUM
    ...
    if (_4p_xfer_count == _4P_XFER_RX_SZ) {
        ...
        if (_4p_rx_packets_to_discard)
            ...
            if (_4p_rx_fifo_WRITE_ptr == RX_FIFO_PTR_END_WRAP_ADDR)
                _4p_rx_fifo_WRITE_ptr = _4p_rx_fifo;
